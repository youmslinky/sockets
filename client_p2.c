...
int main(int argc, char * argv[])
{
	...
	while(1){
		while (fgets(buf, sizeof(buf), stdin)) {
			buf[MAX_LINE-1] = '\0';	//null terminater for string
			len = strlen(buf) + 1;
			send(s, buf, len, MSG_CONFIRM); //send message
			len = recv(s, buf, sizeof(buf), 0); //get message back from server
			printf("SVR_MSG_RCV'd: %s",buf); //pring message recieved from server
